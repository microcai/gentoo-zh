...
src_install() {
	# Preserve original permissions, symlinks and executable bits
	cp -a opt "${ED}" || die

	# Remove unnecessary node-gyp helper directories
	find "${ED}/opt/${PN}" -type d -name "node_gyp_bins" -exec rm -rf {} + 2>/dev/null || die
