...
${GENPATCHES_URI}
https://github.com/HougeLangley/customkernel/releases/download/v5.13-patch/patch-5.13.10-xanmod1
https://github.com/HougeLangley/customkernel/releases/download/v5.13-patch/patch-5.13.10-xanmod1-cacule
...
PATCHES=( "${DISTDIR}/patch-5.13.10-xanmod1"
"${DISTDIR}/patch-5.13.10-xanmod1-cacule"
...
src_prepare() {
	...
	if	use	xanmod	;	then
		eapply "${DISTDIR}/patch-5.13.10-xanmod1"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
	# Enable Xanmod-CaCule
	if	use	cacule	;	then
		eapply "${DISTDIR}/patch-5.13.10-xanmod1-cacule"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
