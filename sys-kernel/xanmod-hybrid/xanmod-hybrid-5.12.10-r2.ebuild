...
S="${WORKDIR}/linux-5.12.10-xanmod-r2"
...
"${DISTDIR}/patch-5.12.10-xanmod1-cacule"
...
src_prepare() {
	...
	if	use	xanmod	;	then
		eapply "${DISTDIR}/patch-5.12.10-xanmod1"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
	# Enable Xanmod-CaCule
	if	use	cacule	;	then
		eapply "${DISTDIR}/patch-5.12.10-xanmod1-cacule"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
