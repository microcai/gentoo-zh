...
${GENPATCHES_URI}
https://github.com/HougeLangley/customkernel/releases/download/v5.14-patch/patch-5.14.1-xanmod1
https://github.com/HougeLangley/customkernel/releases/download/v5.14-patch/patch-5.14.1-xanmod1-cacule
...
PATCHES=( "${DISTDIR}/patch-5.14.1-xanmod1"
"${DISTDIR}/patch-5.14.1-xanmod1-cacule"
"${DISTDIR}/v1-cjktty-5.14.patch" )

src_prepare() {
	# Default enable Xanmod
	if	use	xanmod	;	then
		eapply "${DISTDIR}/patch-5.14.1-xanmod1"	||	die
		eapply "${DISTDIR}/v1-cjktty-5.14.patch"	||	die
	fi
	# Enable Xanmod-CaCule
	if	use	cacule	;	then
		eapply "${DISTDIR}/patch-5.14.1-xanmod1-cacule"	||	die
		eapply "${DISTDIR}/v1-cjktty-5.14.patch"	||	die
	fi
