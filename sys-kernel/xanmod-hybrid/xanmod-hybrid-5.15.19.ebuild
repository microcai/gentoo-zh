...
${GENPATCHES_URI}
https://github.com/HougeLangley/customkernel/releases/download/v5.15-patch/patch-5.15.19-xanmod1
https://github.com/HougeLangley/customkernel/releases/download/v5.15-patch/patch-5.15.19-xanmod1-tt
...
PATCHES=( "${DISTDIR}/patch-5.15.19-xanmod1"
"${DISTDIR}/patch-5.15.19-xanmod1-tt"
"${DISTDIR}/v1-cjktty-5.15.patch" )

src_prepare() {
	# Default enable Xanmod
	if	use	xanmod	;	then
		eapply "${DISTDIR}/patch-5.15.19-xanmod1"	||	die
		eapply "${DISTDIR}/v1-cjktty-5.15.patch"	||	die
	fi
	# Enable Xanmod-CaCule
	if	use	cacule	;	then
		eapply "${DISTDIR}/patch-5.15.19-xanmod1-tt"	||	die
		eapply "${DISTDIR}/v1-cjktty-5.15.patch"	||	die
	fi
