...
${GENPATCHES_URI}
https://github.com/HougeLangley/customkernel/releases/download/v5.12-patch/patch-5.12.13-xanmod1
...
S="${WORKDIR}/linux-5.12.13-xanmod"

K_EXTRAEINFO="For more info on xanmod-hybrid and details on how to report problems,	see: ${HOMEPAGE}."

PATCHES=( "${DISTDIR}/patch-5.12.13-xanmod1"
"${DISTDIR}/patch-5.12.13-xanmod1-cacule"
...
src_prepare() {
	...
	if	use	xanmod	;	then
		eapply "${DISTDIR}/patch-5.12.13-xanmod1"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
	# Enable Xanmod-CaCule
	if	use	cacule	;	then
		eapply "${DISTDIR}/patch-5.12.13-xanmod1-cacule"	||	die
		eapply "${DISTDIR}/v1-cjktty.patch"	||	die
		eapply "${DISTDIR}/v1-uksm.patch"	||	die
	fi
