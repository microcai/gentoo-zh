...
IUSE="+autostart +cairo +dbus debug +glib +gtk +icu +introspection lua
...
src_configure() {
	local mycmakeargs=(
		...
		$(cmake-utils_use_enable gtk GTK3_IM_MODULE)
...
src_compile(){
	...
	if use amd64 && use abi_x86_32 ; then
		...
		use gtk && emake -C frontend/gtk3 || die
...
src_install() {
	if use amd64 && use abi_x86_32 ; then
		...
		use gtk  && emake DESTDIR="${D}" -C frontend/gtk3 install || die
