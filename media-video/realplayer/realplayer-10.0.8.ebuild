...
src_unpack() {
	...
	#only works for linux-x86, amd64 and sparc need a extra file, should be fixed.
	cp -rf distribution\linux-2.2-libc6-gcc32-i586/protocol_dist .
	cp -rf distribution\linux-2.2-libc6-gcc32-i586/datatype_dist .
...
src_compile() {
	...
	${S}/build/bin/build -m realplay_gtk_release -t release -k -p green -P helix-client-all-defines player_all || die
}
