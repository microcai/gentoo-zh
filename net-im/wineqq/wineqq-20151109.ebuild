...
"
PATCHES=( "${FILESDIR}"/${PF}-fix-firstrun.patch )
...
src_install() {
	...
	cd ${D}/opt/longene/qq/
	epatch "${FILESDIR}/${P}-fix-firstrun.patch"
	...
	if use system-wine ; then
		...
		tar -jxf ${D}/opt/longene/qq/wine-lib.tar.bz2 -C ${D}/opt/longene/qq
	fi
	chown -R root:root ${D}
}

pkg_postinst() {
	# setfacl -m g:users:rwx ${D}/opt/longene/qq
	elog
...
# pkg_config() {
# 	setfacl -m g:users:rwx /opt/longene/qq
# }
